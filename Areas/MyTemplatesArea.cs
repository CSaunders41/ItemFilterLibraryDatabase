...
public class MyTemplatesArea : BaseArea
{
    ...
    public MyTemplatesArea(ItemFilterLibraryDatabase plugin, ApiClient apiClient) : base(plugin, apiClient)
    {
        ...
        if (Plugin.Initialized) RefreshTemplates();
    }
    ...
    public override void Draw()
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button("Refresh") && !_isRefreshing) RefreshTemplates();
        ImGui.SameLine();
        if (ImGui.Button("New Template [+]")) _templateModal.Show(null, TemplateModalMode.Create);
        ...
        if (_filteredTemplates.Count > 0)
        {
            ...
            if (ImGui.Button("Previous") && _currentPage > 1) _currentPage--;

            ImGui.SameLine();
            ImGui.Text($"Page {_currentPage} of {totalPages}");

            ImGui.SameLine();
            if (ImGui.Button("Next") && _currentPage < totalPages) _currentPage++;
        ...
        if (_filteredTemplates.Count > 0)
            ...
            ImGui.Text(
                $"No templates found for '{ItemFilterLibraryDatabase.Main.Settings.CurrentTemplateType?.Description ?? "selected type"}'");
    ...
    private void DrawTemplatesTable()
    {
        ...
        if (ImGui.BeginTable("my_templates", 5, flags))
        {
            ...
            foreach (var template in pageTemplates)
            {
                ...
                ImGui.SameLine();
                if (ImGui.Button("Delete", new Vector2(buttonWidth, 0))) DeleteTemplate(template.TemplateId);
    ...
    protected async void RefreshTemplates()
    {
        ...
        try
        {
            ...
            var response =
                await ApiClient.GetAsync<ApiResponse<List<Template>>>(
                    Routes.Templates.GetMyTemplates(currentType.TypeId));
    ...
    private async void ToggleTemplateVisibility(string templateId, bool isPublic)
    {
        try
        {
            ...
            await ApiClient.PatchAsync<ApiResponse<object>>(
                Routes.Templates.ToggleVisibility(currentType.TypeId, templateId), new { is_public = isPublic });
    ...
    private async void DeleteTemplate(string templateId)
    {
        try
        {
            ...
            await ApiClient.DeleteAsync<ApiResponse<object>>(
                Routes.Templates.DeleteTemplate(currentType.TypeId, templateId));
